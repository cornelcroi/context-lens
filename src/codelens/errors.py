...
class ParameterValidationError(KnowledgeBaseError):
    ...
    def __init__(self, message: str, parameter_name: str, parameter_value: Any, 
                 details: Optional[Dict[str, Any]] = None):
        error_details = {
        ...
        super().__init__(
            message=message,
            error_category=ErrorCategory.INVALID_PARAMETER,
            details=error_details
        )
...
def validate_file_path(file_path: str, max_length: int = 4096) -> None:
    ...
    for pattern in dangerous_patterns:
        if pattern in file_path_stripped:
            logger.warning(f"File path contains potentially dangerous pattern '{pattern}': {file_path_stripped}")
...
def validate_limit_parameter(limit: Optional[int], min_value: int = 1, max_value: int = 1000,
                            parameter_name: str = "limit") -> None:
...
def log_operation_failure(operation: str, error: Exception, **kwargs) -> None:
    ...
    logger.error(f"Operation failed: {operation} | Error: {str(error)} | Context: {context_str}", 
                exc_info=error)
