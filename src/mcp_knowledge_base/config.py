...
class EmbeddingConfig:
    ...
    def validate(self) -> None:
        ...
        if self.batch_size <= 0:
            raise ConfigurationError(f"Embedding batch size must be positive, got {self.batch_size}")
        
        if self.batch_size > 1000:
            logger.warning(f"Large embedding batch size ({self.batch_size}) may cause memory issues")
...
class ProcessingConfig:
    ...
    def validate(self) -> None:
        ...
        if self.max_file_size_mb > 100:
            logger.warning(f"Large max file size ({self.max_file_size_mb}MB) may cause memory issues")
        
        if self.chunk_size <= 0:
            raise ConfigurationError(f"Chunk size must be positive, got {self.chunk_size}")
        
        if self.chunk_size < 100:
            logger.warning(f"Small chunk size ({self.chunk_size}) may result in poor search quality")
    ...
    def load(cls, config_file: Optional[str] = None) -> "Config":
        ...
        if config_file:
            if Path(config_file).exists():
                ...
                logger.warning(f"Specified config file not found: {config_file}, falling back to defaults")
