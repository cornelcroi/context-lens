...
class TestLanceDBManager:
    ...
    async def db_manager(self, temp_dir):
        ...
        config = DatabaseConfig(
            path=str(temp_dir / "test_lance.db"),
            table_prefix="test_"
        )
    ...
    async def test_initialization(self, temp_dir):
        ...
        config = DatabaseConfig(
            path=str(temp_dir / "init_test.db"),
            table_prefix="init_"
        )
        manager = LanceDBManager(config)
    ...
    @pytest.mark.asyncio
    async def test_update_existing_document(self, db_manager, sample_document_metadata, sample_chunks):
    ...
    @pytest.mark.asyncio
    async def test_search_vectors_with_limit(self, db_manager, sample_document_metadata, sample_chunks):
    ...
    async def test_database_not_initialized_error(self, temp_dir):
        ...
        config = DatabaseConfig(
            path=str(temp_dir / "uninit_test.db"),
            table_prefix="uninit_"
        )
        manager = LanceDBManager(config)
